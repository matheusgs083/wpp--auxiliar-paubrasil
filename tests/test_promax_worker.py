...
from workers.promax_client import PromaxApiUnavailable, PromaxClient, normalize_status
...
class PromaxClientTests(unittest.TestCase):
    ...
    def test_liga_entrega_upload_uses_reports_root_fallback_without_publication_mapping(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            reports_root = Path(temp_dir) / "RELATORIOS"
            source_dir = reports_root / "03.08.05"
            source_dir.mkdir(parents=True)
            (source_dir / "PATOS_21_09.csv").write_bytes(b"rota;valor\n1;10\n")

            client = Mock()
            client.import_liga_entrega_files.return_value = {
                "ok": True,
                "result": {"routine": "030805_LIGA", "file_count": 1},
            }
            worker = PromaxWorker(
                config=WorkerConfig(
                    api_url="http://localhost:8080",
            ...
            with patch.dict(os.environ, {"PROMAX_LIGA_ENTREGA_REPORTS_ROOT": str(reports_root)}):
                worker._upload_liga_entrega_reports_if_needed(
                    {"payload": {"routines": ["030805_LIGA"], "publish": True}},
                    "job-1",
                    "lease-token",
                    PromaxRunResult(
                        status="success",
...
class PromaxPartialSyncTests(unittest.TestCase):
    def test_partial_result_is_retained_and_retried_after_temporary_api_failure(self) -> None:
        client = Mock()
        client.sync_financeiro_fechamento_mapa.side_effect = PromaxApiUnavailable("temporariamente fora")
        worker = PromaxWorker(
            config=WorkerConfig(
                api_url="http://localhost:8080",
                token="token",
        ...
        self.assertEqual(len(worker._pending_partial_results["job-fechamento"]), 1)
        ...
        self.assertEqual(worker._pending_partial_results["job-fechamento"], [])
...
class PromaxRunnerTests(unittest.TestCase):
    ...
    def test_runner_terminates_process_after_maximum_runtime(self) -> None:
    ...
    def test_runner_uses_km_fallback_as_km_atual_for_fechamento_mapa(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            config = self._config(Path(temp_dir))
            runner = PromaxRunner(config)
    ...
    def test_runner_closes_browser_on_unattended_closure_failure(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            config = self._config(Path(temp_dir))
            runner = PromaxRunner(config)
            command = runner.build_command(
        ...
        self.assertIn("--fechar-ao-falhar", command)
