...
```mermaid
flowchart TB
    host["Host Windows / Docker"] --> app["FastAPI app.py"]
    host --> worker["workers/ Promax Windows"]
    worker --> driver["promax-web-driver"]
    app --> routes["routes/ HTTP e webhooks"]
    app --> services["services/ regras e orquestracao"]
    app --> integrations["integrations/ Evolution, Meta e PayIP"]
    services --> db["PostgreSQL schemas"]
    services --> data["data/ bases locais"]
    services --> exports["exports/ artefatos"]
    routes --> templates["templates/ painel admin"]
    ops["ops/ manutencao e backup"] --> db
    ...
    subgraph deploy_cfg["Deploy e configuracao"]
        deploy["deploy/"]
        evolution["evolution/"]
...
O painel `/admin/promax` controla jobs, fila, agendas e logs do Promax. A API e
o scheduler de manutencao rodam no Docker; a automacao roda no host Windows em
`workers/promax_worker.py`, sempre com concorrencia efetiva de um job.
O comando **Reprocessar publicacoes** adiciona a fila uma tentativa de envio dos
arquivos preservados pelo driver em `logs/publicacao_pendente`, sem baixar os
relatorios novamente.
...
O script:
- gera `PROMAX_WORKER_TOKEN` no `.env` somente quando estiver ausente;
- sobe o Compose;
- inicia o worker minimizado;
- evita uma segunda instancia do worker no mesmo Windows.

Para iniciar junto com o Windows, abra `shell:startup` e crie um atalho para
`start_homelab.bat`. Os detalhes de configuracao e cancelamento por PID estao em
`workers/README.md`.

Durante a homologacao, mantenha a tarefa antiga do Agendador do Windows
desativada apenas nos horarios testados. Remova-a somente depois de uma execucao
completa aprovada pelo painel.
...
- `PROMAX_DATABASE_URL`: conexao do plano de controle Promax.
- `PROMAX_DB_SCHEMA`: schema de jobs, logs, agendas e heartbeat.
- `PROMAX_WORKER_TOKEN`: token exclusivo entre API e worker Windows.
- `PROMAX_WORKER_LEASE_SECONDS`: validade renovavel do job em execucao.
- `PROMAX_SCHEDULER_INTERVAL_SECONDS`: intervalo de agendas e limpeza de leases.
