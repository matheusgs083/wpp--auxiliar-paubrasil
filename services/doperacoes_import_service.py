...
class DOperacoesValidationResult:
    dataset_name: str
    source_path: str
    ok: bool
    total_rows: int
    unique_codigos: int
    unique_tipos_movimento: int
    error_count: int
    warning_count: int
    ...
    def ensure_valid(self) -> None:
        if self.error_count:
            raise ValueError("; ".join(self.sample_errors) if self.sample_errors else "Erros na validacao da tabela de operacoes.")


@dataclass(frozen=True)
class DOperacoesImportSummary:
    source_path: str
    rows: int
    unique_codigos: int
    unique_tipos_movimento: int
    unique_operacoes: int
...
class DOperacoesImportService:
    ...
    def validate_source(self, source_path: Path) -> DOperacoesValidationResult:
        path = source_path.expanduser().resolve()
        errors: list[str] = []
        warnings_list: list[str] = []
        rows: list[DOperacoesRow] = []
        ...
        if not rows and not errors:
            errors.append("Nao encontrei linhas validas na tabela de operacoes.")

        return DOperacoesValidationResult(
            dataset_name="doperacoes",
            source_path=str(path),
            ok=not errors,
            total_rows=len(rows),
            unique_codigos=len({row.codigo for row in rows}),
            unique_tipos_movimento=len({row.tipo_movimento for row in rows}),
    ...
    def summarize_source(self, source_path: Path) -> DOperacoesImportSummary:
        validation = self.validate_source(source_path)
        validation.ensure_valid()
        rows = _load_doperacoes_rows(source_path.expanduser().resolve())
        return DOperacoesImportSummary(
            source_path=str(source_path),
            rows=len(rows),
            unique_codigos=len({row.codigo for row in rows}),
            unique_tipos_movimento=len({row.tipo_movimento for row in rows}),
            unique_operacoes=len({row.nome_operacao for row in rows}),
        )
    ...
    def import_source(self, source_path: Path, reference_date: date | None = None) -> dict[str, Any]:
        ...
        with self._connect() as conn:
            ...
            activate_import_batch(conn, self.schema, "doperacoes", batch_id)
            self._create_latest_view(conn)
            prune_import_batches(conn, self.schema, "doperacoes", keep_last=3)
            conn.commit()
    ...
    def refresh_latest_view(self) -> dict[str, Any]:
        ...
        with self._connect() as conn:
            self._ensure_schema(conn)
            active_batch_id = resolve_effective_import_batch_id(conn, self.schema, "doperacoes", activate_if_missing=True)
            self._create_latest_view(conn)
            conn.commit()
        ...
        return {
            "ok": True,
            "schema": self.schema,
            "view": f"{self.schema}.doperacoes_latest",
    ...
    def _ensure_schema(self, conn: psycopg.Connection[Any]) -> None:
        with conn.cursor() as cur:
            ...
            cur.execute(
                sql.SQL(
                    """
                    CREATE TABLE IF NOT EXISTS {}.doperacoes_snapshot (
            ...
            cur.execute(
                sql.SQL(
                    "CREATE INDEX IF NOT EXISTS doperacoes_snapshot_batch_codigo_idx ON {}.doperacoes_snapshot (batch_id, codigo)"
                ).format(sql.Identifier(self.schema))
            )
            cur.execute(
                sql.SQL(
                    "CREATE INDEX IF NOT EXISTS doperacoes_snapshot_batch_tipo_idx ON {}.doperacoes_snapshot (batch_id, tipo_movimento)"
                ).format(sql.Identifier(self.schema))
            )
            cur.execute(
                sql.SQL(
                    "CREATE INDEX IF NOT EXISTS import_batches_doperacoes_dataset_idx ON {}.import_batches (dataset_name, imported_at DESC)"
                ).format(sql.Identifier(self.schema))
            )
    ...
    def _insert_snapshot_rows(
        ...
        query = sql.SQL(
            """
            INSERT INTO {}.doperacoes_snapshot (
                batch_id,
                row_number,
                codigo,
                tipo_movimento,
    ...
    def _create_latest_view(self, conn: psycopg.Connection[Any]) -> None:
        ...
        query = sql.SQL(
            """
            CREATE OR REPLACE VIEW {}.doperacoes_latest AS
            SELECT
                s.batch_id,
                s.row_number,
                s.codigo,
...
def _load_doperacoes_rows(path: Path) -> list[DOperacoesRow]:
    ...
    if len(header) < 3:
        raise ValueError("Arquivo invalido. Informe pelo menos as 3 primeiras colunas: Codigo, Tipo Movimento e Nome da Operacao.")

    rows: list[DOperacoesRow] = []
    for row_number, row in enumerate(reader, start=2):
        if not row or not any(str(value or "").strip() for value in row):
            continue
        codigo = _clean_code(row[0] if len(row) > 0 else "")
        tipo_movimento = _clean_code(row[1] if len(row) > 1 else "")
        nome_operacao = _clean_text(row[2] if len(row) > 2 else "")
        if not codigo or not tipo_movimento or not nome_operacao:
            continue
        rows.append(
            DOperacoesRow(
                codigo=codigo,
