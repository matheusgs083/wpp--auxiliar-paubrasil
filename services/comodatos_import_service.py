...
class ComodatosImportService:
    ...
    def import_source(self, source_path: Path, reference_date: date | None = None) -> dict[str, Any]:
        ...
        with self._connect() as conn:
            ...
            self._replace_dataset_contents(conn, dataset_name="comodatos")
    ...
    def refresh_latest_view(self) -> dict[str, Any]:
        ...
        return {"ok": True, "schema": self.schema, "view": f"{self.schema}.comodatos_latest"}
    ...
    def _create_latest_view(self, conn: psycopg.Connection[Any]) -> None:
        drop_query = sql.SQL("DROP VIEW IF EXISTS {}.comodatos_latest").format(sql.Identifier(self.schema))
        query = sql.SQL(
            """
