...
def main() -> None:
    ...
    try:
        validation = service.validate_workbook(args.file_path)
        print(json.dumps({"mode": "validate", **validation.to_dict()}, ensure_ascii=False, indent=2))
        validation.ensure_valid()

        summary = service.summarize_workbook(args.file_path)
        ...
        result = service.import_workbook(args.file_path, reference_date=batch_date)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Analisa e importa a planilha de giro para PostgreSQL no formato flat normalizado."
    )
    parser.add_argument(
        "file_path",
        nargs="?",
        type=Path,
        default=Path("data/Giro/giro.xlsx"),
        help="Caminho da planilha de giro.",
    )
    parser.add_argument(
        "--import-db",
        action="store_true",
        help="Alem da analise, importa a planilha para o banco configurado.",
    )
    parser.add_argument(
        "--reference-date",
